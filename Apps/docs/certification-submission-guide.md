---
ms.author: oromalle
title: Microsoft 365 сертификации Руководство по отправке
author: orionomalley
manager: tonybal
...
ms.locfileid: "64461732"
---
# <a name="microsoft-365-certification-submission-guide"></a>Microsoft 365 сертификации Руководство по отправке

**В этой статье:**
- [Введение](#introduction)
- [Необходимые компоненты](#prerequisites) 
- [Microsoft 365 обновления спецификации сертификации](#microsoft-365-certification-specification-updates)
...
В рамках Microsoft 365 сертификация Microsoft 365 предоставляет корпоративным организациям уверенность в том, что данные и конфиденциальность надлежащим образом защищены и защищены при интеграции сторонних приложений-разработчиков и надстроек в платформу Microsoft 365. Приложения и надстройки, которые проходят проверку, будут Microsoft 365 сертифицированы в Microsoft 365 экосистеме. 
...
Этот документ предназначен для ПОСТАВЩИКОВ (независимых поставщиков программного обеспечения) для предоставления информации о процессе сертификации Microsoft 365, необходимых для запуска процесса и сведений о конкретных средствах управления безопасностью, которые должны иметься в isVs.  Общие сведения о программе Microsoft 365 приложения можно найти на странице Microsoft 365 [приложения.](../overview.md) 
...
Перед присуждением Microsoft 365 сертификации необходимо завершить Publisher аттестацию. Однако вы можете начать процесс Microsoft 365 сертификации до завершения Publisher аттестации.  

### <a name="read-the-microsoft-365-certification-specification"></a>Ознакомьтесь с Microsoft 365 сертификации
...
## <a name="microsoft-365-certification-specification-updates"></a>Microsoft 365 обновления спецификации сертификации 
...
1. Перейдите в центр партнеров и просмотрите завершенную документацию [Publisher проверки](../docs/attestation.md). При необходимости можно изменить и обновить ответы; однако, если вы это сделаете, вам потребуется повторно предоставить документацию по проверке для утверждения. Если срок отправки превышает три месяца, мы требуем повторного Publisher проверки и проверки. 
1. Внимательно ознакомьтесь с руководством [по Microsoft 365 сертификации](../docs/certification-submission-guide.md), чтобы понять, что от вас потребуется. Убедитесь, что вы сможете выполнить требования [](../docs/certification-submission-guide.md#app-certification-criteria) по контролю, указанные в руководстве по сертификации Microsoft 365 сертификации.
...
>[!NOTE]
>Для всех приложений office вы можете ссылаться [на наше руководство Office apps User Guide](../docs/userguide.md). Для всех веб-приложений вы можете ссылаться на руководство [пользователя приложения SaaS](../docs/saasuserguide.md).
...
1. После проверки отправки аналитиком вы будете уведомлены о решении по сертификации. Приложения, удостоенные сертификата, получат значок в приложении на страницах **AppSource** и **Microsoft docs** . Полную информацию о преимуществах сертификации можно прочитать [здесь](../docs/enterprise-app-certification-guide.md#program-benefits).
...
|**С помощью схем потока данных** |Flow схемы, подробные ниже.
||&#x2713; M365 Data поступает в Приложение и надстройку (включая [EUII](#euii) и [OII](#oii) ).|
||&#x2713; M365 Потоки данных в вспомогательной инфраструктуре (где это применимо)|
...
|**Подтверждение соответствия требованиям**|Поддержка документации для внешних структур безопасности, включенных в Publisher аттестации, или должна учитываться при рассмотрении Microsoft 365 элементов управления сертификацией. В настоящее время поддерживаются следующие три:|
||&#x2713; [PCI DSS](#pci-dss) attestation of Compliance (AOC).|
||&#x2713; [отчеты SOC 2](#soc-2) Типа I/Type II.|
||&#x2713; [ISMSIEC](#isms) /  — заявление о применимости и сертификации 1S0/IEC 27001.[](#iec)|
...
Используемые методы сбора доказательств будут определены в процессе оценки. Конкретные примеры типа доказательств, необходимых в представлении, см. в руководстве [по примеру доказательств](../docs/certification-sample-evidence-guide.md).
...
Если это возможно, и чтобы сократить время, необходимое для завершения оценки, [](#initial-document-submission)  любая документация или вся документация, описанная в отчете о документации, должна быть предоставлена заранее.

Аналитики по сертификации сначала проверят доказательства, представленные в первоначальной документации, и сведения о Publisher аттестации, чтобы определить соответствующие строки запроса, размер выборки и необходимость получения дополнительных доказательств, как описано выше.  Аналитики сертификации проанализируют всю собранную информацию, чтобы сделать выводы о том, как и как и если вы Microsoft 365 элементов управления. 
...
Каждый из этих доменов безопасности включает в себя определенные элементы управления ключами, охватывающие одно или несколько конкретных требований, которые будут оцениваться в рамках процесса оценки. Чтобы обеспечить Microsoft 365 сертификации для разработчиков всех размеров, каждый из доменов безопасности оценивается с помощью системы скоринга, чтобы определить общую оценку по каждому из доменов. Оценки для каждого из элементов Microsoft 365 сертификации распределяются между 1 (низким) и 3 (высокими) в зависимости от предполагаемого риска неустойки этого контроля. Каждый из доменов безопасности будет иметь минимальную процентную отметку, которая будет считаться пропуском. Некоторые элементы этой спецификации включают некоторые автоматические критерии сбой:
...
Проверка разрешений GraphAPI проводится для проверки того, что приложение/надстройка не запрашивает чрезмерно разрешительных разрешений. Это осуществляется вручную, проверяя, какие разрешения запрашиваются. Аналитики сертификации перекрестят эти проверки с представлением аттестации Publisher и оценят уровень запрашиваемого доступа для обеспечения того, чтобы практики "наименьших привилегий" были выполнены. Если аналитики сертификации считают, что эти методы "наименьших привилегий" не выполнены, аналитики сертификации будут открыто обсуждать с вами, чтобы проверить бизнес-обоснование запрашиваемой разрешения. Любые несоответствия по Publisher, найденные во время этого обзора, также получат обратную связь, Publisher проверку можно обновить. 
...
Тестирование на проникновение  должно проводиться в среде живого производства, которая поддерживает развертывание приложения/надстройки (например, где размещен код приложения/надстройки, который обычно является ресурсом в файле манифеста) наряду с любой дополнительной средой, поддерживающей работу приложения и надстройки (например, если приложение/надстройка разговаривает с другими веб-приложениями за пределами Microsoft 365).  При определении области необходимо принять меры по обеспечению того, чтобы все "подключенные" системы или среды, которые могут повлиять на безопасность среды в области, также были включены в все действия по тестированию на проникновение. 
...
Отчеты о тестировании на проникновение будут рассмотрены для обеспечения того, чтобы не было уязвимостей ****, которые соответствуют следующим критериям отказа, описанным в нижеупольных элементов управления.
...
|| Наличие раскрытия исходных кодов ( [включаяLFI](#lfi)).|
...
**Требования и правила тестирования на бесплатное проникновение**

- Для isVs, которые в настоящее время не участвуют в тестировании на проникновение, тестирование на проникновение может быть проведено бесплатно Microsoft 365 сертификации. Корпорация Майкрософт организует и покрывает затраты на тестирование на проникновение в течение до 12 дней ручного тестирования. Затраты на тесты на проникновение рассчитываются в зависимости от количества дней, необходимых для проверки среды. Ответственность за любые расходы, превышающие 12 дней тестирования, несет isV. 
- Перед проводимым тестом на проникновение isVs должны будут предоставить доказательства и получить одобрение для 50% элементов управления в области. Чтобы начать работу, просто заполните исходное представление документов и включите тестирование на проникновение в рамках оценки. При заполнении 50% элементов управления вам будет задана возможность связаться с областью и запланировать тест на проникновение.
- Отчет, выданный после завершения проверки, будет предоставлен isV после завершения сертификации. Этот отчет вместе с Microsoft 365 сертификацией можно использовать для демонстрации потенциальным клиентам безопасности среды.
- Кроме того, isVs будут отвечать за демонстрацию того, что уязвимости, выявленные в тесте на проникновение, были исправлены до получения сертификата, но не нужно создавать чистый отчет.
...
| ------------------------|------------------------------ |
| **Защита от вредоносных программ — антивирус**|Предокавляй документацию по политике, которая управляет антивирусной практикой и процедурами.|
...
|**Элементы управления** приложениями: требуется только в том случае, если традиционные антивирусные программы не используются|Предокабельное подтверждение того, что приложения утверждены до развертывания.|
...
|**Управление учетной записью**| Предоставление документации по политике, которая регулирует методы и процедуры управления учетной записью.
...
||Предоставление демонстрации того, что уникальные учетные записи пользователей выданы всем пользователям.|
||Предоказательное доказательство того, что в среде следуют принципы наименьших привилегий.|
||Предокавказательное доказательство того, что существует процесс обеспечения безопасности или ужесточить учетные записи службы и за этим процессом следует.|
||Предоставление демонстрационных данных о том, что MFA настроена для всех подключений к удаленному доступу и для всех неконсоляционных административных интерфейсов.|
||Предоставление демонстрационных доказательств того, что сильное шифрование настроено для всех подключений к удаленному доступу и для всех неконсоляционных административных интерфейсов, включая доступ к любым репозиториям кода и интерфейсам управления облаками.|
||Предоставление демонстрационной информации о том, что MFA используется для защиты портала администрирования, который используется для управления и поддержания записей всех записей службы имен общего домена (DNS).|
...
||Предокабельные доказательства, показывающие, что сотрудники всегда доступны в течение всего дня и каждый день для реагирования на оповещения о безопасности.|
...
Если приложение извлекает и хранит данные M365, вам потребуется реализовать схему шифрования хранения данных, которая будет следовать спецификации, определенной в [**приложении B**](#appendix-a).

### <a name="controls"></a>Элементы управления

|**Управление семейство**| **Controls** |
| -----------------------|-------------------------------- |
|**Данные в транзите**| Предоставление наглядного подтверждения того, что конфигурация TLS соответствует требованиям шифрования в требованиях конфигурации профилей [TLS](../docs/certification-submission-guide.md#appendix-a) или превышает их.|
...
|**Хранение и реализация данных**|Предоставление наглядного подтверждения того, что официально установлен утвержденный и задокументированный период хранения данных.|
...
Если внешние рамки безопасности включены в Publisher аттестации, аналитикам по сертификации необходимо проверить правильность этих рамок соответствия требованиям безопасности в рамках оценки Microsoft 365 сертификации.

|**Платформа** | **Дополнительные рекомендации** |
|-------------- | --------------------|
|ISO 27001| [**Приложение C**](#appendix-c). Коллекция доказательств — deltas для ISO 27001.|
...
> [!NOTE]
> Хотя указанные выше внешние стандарты и рамки безопасности могут быть представлены в качестве доказательств для удовлетворения некоторых элементов управления сертификацией Microsoft 365, прохождение сертификации Microsoft 365 не означает, что вы успешно пройдете аудит в отношении этих стандартов и рамок. Спецификация Microsoft 365 сертификации — это лишь небольшой подмножество этих стандартов и рамок безопасности, что позволяет Корпорации Майкрософт получить уровень уверенности в связи с вашей позицией безопасности.
...
&emsp;&#x2713; допускается ECDH. Ключ **должен** быть не менее 256 битов. Необходимо использовать кривой NIST P-256, P-384 или P-521.
...
* Показано, что антивирус настроен во всех примерных компонентах системы, чтобы автоматически блокировать вредоносные программы, & оповещения или оповещения.
...
* Подтверждение поддержки документации по исправлению уязвимости на основе ранжирования риска и в соответствии со спецификацией следующим образом:
 ...
 &#x2713; устранение всех критических, высоких и средних проблем риска в соответствии с ранжированием риска для внешнего сканирования.
...
&#x2713;, запрос контрабанды и разделения ответов.
...
&#x2713; сведения о том, какие функциональные возможности должны быть проведены.

&#x2713; сведения о любых процедурах обратного отъема.
...
&#x2713; IRP охватывает все системы.
 
&#x2713; подготовки для группы реагирования на инциденты.
...
В тех случаях, когда вы уже достигли соответствия требованиям PCI DSS, в рамках этого Microsoft 365 сертификации необходимо как минимум проанализировать следующие пробелы (пробелы), полностью не охваченные PCI DSS.

> [!NOTE]
> В рамках оценки Microsoft 365 сертификации аналитик сертификации определит, не включены ли какие-либо из элементов управления PCI DSS в оценку PCI DSS, а также может принять решение о примере элементов управления, которые были признаны включенными для предоставления дополнительной гарантии. Все требования, отсутствующие в PCI DSS, должны быть включены в Microsoft 365 сертификации.
...
В тех случаях, когда вы уже достигли соответствия требованиям SOC 2, в рамках этого Microsoft 365 сертификации необходимо будет просмотреть следующие пробелы (пробелы), полностью не охваченные soC 2.

> [!NOTE]
> В рамках оценки Microsoft 365 сертификации аналитик сертификации определит, не включены ли какие-либо элементы управления SOC 2 в качестве части оценки SOC 2, а также может принять решение о примере элементов управления, которые были найдены для обеспечения дополнительной гарантии. Все требования, отсутствующие в оценке SOC 2, должны быть включены в Microsoft 365 сертификации.
...
&emsp;&#x2713;, запрос контрабанды и разделения ответов.
...
&emsp;&#x2713; индикатор успеха и отказа.
