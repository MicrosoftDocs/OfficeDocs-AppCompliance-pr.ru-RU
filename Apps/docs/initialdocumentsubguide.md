...
## <a name="what-is-the-hosting-environment-or-service-model-used-to-run-your-app"></a>Какова среда размещения или модель службы, используемая для запуска приложения?
- Инфраструктура как служба (IaaS) — это модель облачной службы, в которой поставщик облачных служб размещает компоненты инфраструктуры, но ПОСТАВЩИКИ по-прежнему отвечают за развертывание и управление компонентами по отдельности, такими как виртуальные машины и операционные системы, хранилища данных и компоненты сетей. Примерами этого являются Azure Virtual Machine и Azure Disk служба хранилища.
- Platform as a Service (PaaS) — это модель облачных служб, в которой компоненты инфраструктуры управляются поставщиком облачных служб. IsVs отвечают только за развертывание собственных приложений и служб. Примерами этого являются службы приложений Azure, Azure Functions и Azure CDN.
- В этом контексте isV hosted означает, что не используется поставщик облачных служб. IsV физически управляет собственными серверами, дисками, сетевыми сетями самостоятельно на локальной основе.
- Гибрид в этом контексте означает, что используется одна из вышеперечисленных моделей. Например, некоторые провайдеры могут использовать смесь служб IaaS и Службы PaaS для поддержки своего приложения, или у них могут быть собственные компоненты isV hosted, а другие передаются поставщику облачных служб. Если вы используете еще одну модель службы, выберите гибрид.
...
|-|-|-|-|-|
|D212|  Windows Machine|   Виртуальная машина|    Н/Д| Н/Д|
