...
|Службы IIS|Корпорация Майкрософт|10.0|Поддерживает интерфейсы API.|
...
## <a name="external-certifications-soc2-pci-dss-iso27001---optional"></a>Внешние сертификаты (SOC2, PCI DSS, ISO27001) — необязательно

Если вы уже получили сертификат SOC2, PCI DSS или ISO27001 и получили отчет за последние 12 месяцев, который не включает полную область сертификации приложения, а также вспомогательную среду, вы можете отправить его во время первоначальной отправки документа. Мы попытаем использовать его для удовлетворения подмножества элементов управления и ускорения оценки. Однако это не обязательно для получения сертификата Microsoft 365 сертификации. 
