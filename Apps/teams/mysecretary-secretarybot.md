...
>|:----------------|:--------------------|:---------------------------------------------------|:--------------------------|:--------------------------|
>| Calendars.Read.Shared | делегирована |  | Извлечение сведений о свободном времени пользователя и его коллег. |  |
>| Calendars.ReadWrite | делегирована |  | Отправка запроса на собрание вместо пользователя. |  |
>| MailboxSettings.Read | делегирована | Хранить язык для показа правильного лангажа. Сохранение зоны времени для правильного вызова API Graph календаря | Извлечение параметров языка и часового пояса пользователя. |  |
>| People.Read | делегирована |  | Попробуйте найти коллег, которые имеют прочные отношения с пользователем. |  |
...
>| email | делегирована | описание выше. | Для хранения электронной почты. |  |
>| openid | делегирована |  | Для проверки подлинности OpenID. |  |
>| profile | делегирована | Сохранение OID для идентификации уникального идентификатора пользователя в системе удостоверений MS. | Получение имени пользователя и OID. Попробуйте использовать OID для подключения Outlook addin в будущем. |  |
